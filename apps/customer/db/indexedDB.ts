import { 
  Tenant, MenuItem, Order, OrderItemSummary, RiderJob, Promotion, SupportTicket, 
...
class UmunthuhubStorageService {
  ...
  addOrder(order: Order): Order {
    ...
    const newJob: RiderJob = {
      ...
      distanceKm: 2.8,
      estimatedMinutes: 16,
      itemsCount: order.items.reduce((acc: number, i: OrderItemSummary) => acc + i.quantity, 0),
      itemsList: order.items.map((i: OrderItemSummary) => `${i.quantity}x ${i.name}`),
      expiresAt: Date.now() + 45 * 1000,
      status: 'accepted',
      checklist: order.items.map((i: OrderItemSummary, idx: number) => ({
        id: `c-${idx}`,
  ...
  toggleJobChecklistItem(jobId: string, checkId: string): void {
    const job = this.memoryCache.riderJobs.find(j => j.id === jobId);
    if (job) {
      const item = job.checklist.find((c: { id: string; label: string; checked: boolean }) => c.id === checkId);
