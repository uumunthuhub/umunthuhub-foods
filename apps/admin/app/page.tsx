...
const MainAppContent: React.FC = () => {
  ...
  // Admin Flow State
  const [adminViewMode, setAdminViewMode] = React.useState<
    "store_selector" | "dashboard"
  >("dashboard");
  const [wizardStep, setWizardStep] = React.useState<number>(1);

  const handlePersonaSelect = (
    selectedPersona: "vendor" | "rider" | "admin" | "corporate",
  ) => {
  ...
  const handleAdminAuthSuccess = (
    mode: "existing" | "new_no_org" | "incomplete_step",
    userEmail?: string,
  ) => {
    if (mode === "existing") {
      // Get the logged-in user and set their organization ID
      const user = userEmail
        ? dbService.findUserByEmail(userEmail)
        : dbService.getAdminUsers()[0];
    ...
    if (persona === "vendor") {
      switch (vendorTab) {
        ...
        case "setup_wizard":
          return (
            <StoreSetupWizard
              onComplete={() => setVendorTab("kds")}
              onCancel={() => setVendorTab("dashboard")}
            />
          );
        default:
          return <VendorDashboard />;
    ...
    if (persona === "admin") {
      return (
        <div className="space-y-4">
          ...
          {adminTab === "store_wizard" && (
            <StoreSetupWizard
              onComplete={() => setAdminTab("overview")}
              onCancel={() => setAdminTab("overview")}
            />
          )}
          {adminTab === "auth" && (
            <AdminAuthView
              initialMode="login"
              onLoginSuccess={(mode) => handleAdminAuthSuccess(mode)}
            />
          )}
          {adminTab === "store_selector" && (
  ...
  return (
    <>
      ...
      <div
        className={`flex-1 font-body selection:bg-[#ab3500] selection:text-white transition-colors duration-300 overflow-x-hidden ${
          themeMode === "dark"
            ? "bg-[#1a1c1c] text-[#f5f5f5]"
      ...
      >
            ...
            <div className="flex-1 flex items-center">
              <div className="w-full">
                <AdminAuthView
                  ...
                  onLoginSuccess={(mode, email) =>
                    handleAdminAuthSuccess(mode, email)
                  }
                />
              </div>
            </div>
        ...
        {/* ── Main Portal ── */}
        {appScreen === "portal" &&
          !(persona === "admin" && adminViewMode === "store_selector") && (
        ...
        {toasts.length > 0 && (
          <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2 max-w-sm pointer-events-none">
            {toasts.map((toast) => (
              ...
              >
                {toast.type === "success" ? (
                  <CheckCircle className="w-5 h-5 text-[#00ae81]" />
                ) : toast.type === "warning" ? (
