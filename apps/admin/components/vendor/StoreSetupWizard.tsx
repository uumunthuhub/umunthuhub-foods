...
export const StoreSetupWizard: React.FC<StoreSetupWizardProps> = ({ initialStep = 1, onComplete, onCancel }) => {
  const { addTenantVenue, addMenuItem, setCurrentTenantId, setPersona, setVendorTab, setAdminTab, showToast, themeMode } = useApp();
  ...
  const [formData, setFormData] = useState({
    ...
    tagline: 'Hand-stretched sourdough pizzas fired at 900°F in authentic volcanic stone',
  ...
  const handleFinalSubmit = () => {
    ...
    addTenantVenue({
      ...
      tags: [formData.cuisine, 'Fresh Ingredients', 'Chef Signature', 'New Arrival'],
      isOpen: true,
    ...
    if (formData.starterItem1Name) {
      addMenuItem({
        ...
        description: 'Authentic stone-oven baked with fresh San Marzano tomatoes and basil.',
    ...
    if (formData.starterItem2Name) {
      addMenuItem({
        ...
        description: 'Warm artisan crust brushed with roasted garlic butter and fresh thyme.',
    ...
    showToast('Store Launched Successfully! 🎉', `${formData.name} is now live and accepting orders in the Umunthuhub-Foods ecosystem.`, 'success');
  ...
  const stepsHeader = [
    ...
    { num: 2, title: 'Location & Delivery', desc: 'Address, radius, min order' },
  ...
  return (
    <div className={`max-w-6xl mx-auto space-y-6 pb-20 ${
      ...
      {/* Header */}
      <div className={`rounded-3xl p-7 border flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
        themeMode === 'dark' 
        ...
        <div>
          ...
          <p className={`text-sm mt-1 ${
            ...
            Provision a new multi-tenant restaurant branch, configure delivery dispatch, and launch your digital kitchen.
          </p>
        </div>
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-extrabold ${
                    isCurrent ? 'bg-[#ab3500] text-white' : isCompleted ? 'bg-[#00ae81] text-white' : themeMode === 'dark' ? 'bg-[#3a3a3a] text-[#c4c4c4]' : 'bg-[#e1bfb5] text-[#594139]'
                  }`}>
                    {isCompleted ? '✓' : s.num}
                  </span>
      ...
      {/* Form Content Body */}
      <div className={`rounded-3xl p-10 border space-y-7 ${
        themeMode === 'dark' 
          ? 'bg-[#242625] border-[#3a3a3a]/50' 
          : 'glass-panel border-[#e1bfb5]/50'
      }`}>
          ...
          <div className="space-y-5 animate-in fade-in duration-200">
            <div>
              <h3 className={`font-heading font-extrabold text-xl ${
                themeMode === 'dark' ? 'text-[#f5f5f5]' : 'text-[#1a1c1c]'
              }`}>1. Restaurant Identity & Brand Assets</h3>
              <p className={`text-sm ${
                themeMode === 'dark' ? 'text-[#c4c4c4]' : 'text-[#594139]'
              }`}>How your store will appear to hungry diners across the Umunthuhub-Foods marketplace</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                >  
                  <option value="Woodfired Pizza">Woodfired Pizza & Italian</option>
                  <option value="Artisan Bowls">Artisan Grain & Salad Bowls</option>
                  <option value="Japanese Sushi">Japanese Sushi & Ramen</option>
                  <option value="Gourmet Burgers">Gourmet Smash Burgers</option>
                  <option value="Artisan Bakery">Artisan Bakery & Pastries</option>
                  <option value="Mexican Street Food">Mexican Taqueria & Street Food</option>
                  <option value="Mediterranean Grill">Mediterranean & Levantine Grill</option>
                  <option value="Thai & Pan-Asian">Thai & Pan-Asian Curries</option>
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              <div className="space-y-2">
                <label className={`font-heading font-bold text-sm ${
                  themeMode === 'dark' ? 'text-[#f5f5f5]' : 'text-[#1a1c1c]'
                }`}>Store Brand Logo URL</label>
                <div className="flex items-center gap-3">
                  <img src={formData.logo} alt="Preview" className={`w-16 h-16 rounded-2xl object-cover border shadow-sm shrink-0 ${
              ...
              <div className="space-y-2">
                <label className={`font-heading font-bold text-sm ${
                  themeMode === 'dark' ? 'text-[#f5f5f5]' : 'text-[#1a1c1c]'
                }`}>Store Banner Hero URL</label>
                <div className="flex items-center gap-3">
                  <img src={formData.banner} alt="Banner Preview" className={`w-24 h-16 rounded-2xl object-cover border shadow-sm shrink-0 ${
                    themeMode === 'dark' ? 'border-[#3a3a3a]' : 'border-[#e1bfb5]'
                  }`} />
                  <input
                    type="text"
        ...
        {step === 2 && (
          <div className="space-y-5 animate-in fade-in duration-200">
            <div>
              <h3 className={`font-heading font-extrabold text-xl ${
                themeMode === 'dark' ? 'text-[#f5f5f5]' : 'text-[#1a1c1c]'
              }`}>2. Physical Kitchen Location & Delivery Zones</h3>
              <p className={`text-sm ${
                themeMode === 'dark' ? 'text-[#c4c4c4]' : 'text-[#594139]'
              }`}>Dispatch center coordinates and order dispatch radii</p>
            </div>
            ...
            <div className={`p-5 rounded-2xl border flex items-center justify-between ${
              ...
              <div>
                ...
                <p className={`text-xs ${
                  themeMode === 'dark' ? 'text-[#c4c4c4]' : 'text-[#594139]'
                }`}>Orders below this threshold will prompt an extra fee alert</p>
              </div>
        ...
        {step === 3 && (
          <div className="space-y-5 animate-in fade-in duration-200">
            <div>
              <h3 className={`font-heading font-extrabold text-xl ${
                themeMode === 'dark' ? 'text-[#f5f5f5]' : 'text-[#1a1c1c]'
              }`}>3. Kitchen Prep Speed & Service Schedule</h3>
              <p className={`text-sm ${
                themeMode === 'dark' ? 'text-[#c4c4c4]' : 'text-[#594139]'
              }`}>Configuring ticket wait times and opening hours for courier dispatch algorithms</p>
            </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={e => setFormData({ ...formData, prepTimeAvg: Number(e.target.value) })}
                    className={`w-full px-4 py-3 rounded-xl text-sm font-bold text-center ${
                      themeMode === 'dark' 
                        ? 'bg-[#383a39] border-[#3a3a3a] text-[#f5f5f5]' 
                        : 'glass-input'
        ...
        {step === 4 && (
          <div className="space-y-5 animate-in fade-in duration-200">
            <div>
              <h3 className={`font-heading font-extrabold text-xl ${
                themeMode === 'dark' ? 'text-[#f5f5f5]' : 'text-[#1a1c1c]'
              }`}>4. Direct Payouts & Commercial Food KYC</h3>
              <p className={`text-sm ${
                themeMode === 'dark' ? 'text-[#c4c4c4]' : 'text-[#594139]'
              }`}>Settlement routing and local public health inspection certificates</p>
            </div>

            <div className="p-5 bg-[#00ae81]/10 rounded-2xl border border-[#00ae81]/30 flex items-center gap-3">
              <Check className="text-[#006c4f] w-[28px] h-[28px]" />
              <div>
                <p className="font-heading font-bold text-sm text-[#006c4f]">Stripe Connect Instant Escrow Enabled</p>
                <p className="text-xs text-[#594139]">Daily automated ACH transfers directly to your restaurant bank account.</p>
              </div>
            </div>
            ...
            <div className="space-y-1">
              <label className={`font-heading font-bold text-sm ${
                themeMode === 'dark' ? 'text-[#f5f5f5]' : 'text-[#1a1c1c]'
              }`}>Legal Entity / Business Name *</label>
              <input
                type="text"
                required
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className={`font-heading font-bold text-sm ${
                  themeMode === 'dark' ? 'text-[#f5f5f5]' : 'text-[#1a1c1c]'
                }`}>Sanitation Inspection Status</label>
                <input
                  type="text"
                  value={formData.sanitationGrade}
                  onChange={e => setFormData({ ...formData, sanitationGrade: e.target.value })}
          ...
          <div className="space-y-5 animate-in fade-in duration-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <h3 className={`font-heading font-extrabold text-xl ${
                  themeMode === 'dark' ? 'text-[#f5f5f5]' : 'text-[#1a1c1c]'
                }`}>5. Initial Menu Dishes & Staff Invite</h3>
                <p className={`text-sm ${
                  themeMode === 'dark' ? 'text-[#c4c4c4]' : 'text-[#594139]'
                }`}>Seed your live digital catalog and invite kitchen team members (optional)</p>
              </div>

              <button
                type="button"
                onClick={() => {
                  setFormData({ ...formData, starterItem1Name: '', starterItem2Name: '', skipSampleProducts: true });
                  showToast('Products & Staff Skipped', 'You can populate menu items anytime from the Store Console.', 'info');
                  handleFinalSubmit();
                }}
            ...
            <div className={`p-5 rounded-2xl border space-y-4 ${
              ...
              <h4 className={`font-heading font-bold text-sm flex items-center gap-2 ${
                ...
                <Utensils className="w-[20px] h-[20px] text-[#ab3500]" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="sm:col-span-2 space-y-1">
                  ...
                  <input
                    ...
                    onChange={e => setFormData({ ...formData, starterItem1Name: e.target.value })}
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={e => setFormData({ ...formData, starterItem1Price: Number(e.target.value) })}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="sm:col-span-2 space-y-1">
                  ...
                  <input
                    ...
                    onChange={e => setFormData({ ...formData, starterItem2Name: e.target.value })}
                    className={`w-full px-4 py-2.5 rounded-xl text-sm font-semibold ${
                      themeMode === 'dark' 
                        ? 'bg-[#383a39] border-[#3a3a3a] text-[#f5f5f5] placeholder-[#7a7a7a]' 
                        : 'glass-input'
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={e => setFormData({ ...formData, starterItem2Price: Number(e.target.value) })}
                    className={`w-full px-4 py-2.5 rounded-xl text-sm font-bold text-center ${
                      themeMode === 'dark' 
                        ? 'bg-[#383a39] border-[#3a3a3a] text-[#f5f5f5]' 
                        : 'glass-input'
            ...
            <div className={`p-5 rounded-2xl border space-y-3 ${
              ...
              <h4 className={`font-heading font-bold text-sm flex items-center gap-2 ${
                ...
                <UserPlus className="w-[20px] h-[20px] text-[#24619d]" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <input
                  ...
                  onChange={e => setFormData({ ...formData, initialChefEmail: e.target.value })}
                ...
                <select
                  ...
                  onChange={e => setFormData({ ...formData, initialChefRole: e.target.value })}
                  className={`w-full px-4 py-2.5 rounded-xl text-sm font-semibold ${
                    themeMode === 'dark' 
                      ? 'bg-[#383a39] border-[#3a3a3a] text-[#f5f5f5]' 
                      : 'glass-input bg-white'
        ...
        <div className={`pt-6 border-t flex items-center justify-between gap-4 ${
          ...
          <button
            type="button"
            onClick={handleNext}
            className="px-8 py-3 rounded-2xl glass-button-primary font-heading font-bold text-sm shadow-lg shadow-[#ab3500]/25 transition-all cursor-pointer flex items-center gap-2"
          >
            <span>{step === totalSteps ? '🚀 Complete & Launch Store' : 'Continue to Next Step →'}</span>
          </button>
        </div>
