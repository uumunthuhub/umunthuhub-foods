...
export const Header: React.FC = () => {
  ...
  const getBackOfficeTitle = () => {
    if (persona === "admin") {
      switch (adminTab) {
        ...
        case "venues":
          return "Store & Multi-Venue Management";
        case "team":
          return "Team Roster & Access Control";
        case "billing":
          return "SaaS Plan & Commission Billing";
        case "support":
          return "Help Desk & Live Inquiries";
        case "store_wizard":
          return "Provision New Store Wizard";
        default:
          return "Platform Admin HQ";
      }
    }
    if (persona === "vendor") {
      switch (vendorTab) {
        case "dashboard":
          return `${currentTenant.name} • Overview`;
        case "kds":
          return `${currentTenant.name} • Kitchen Display (KDS)`;
        case "menu":
          return `${currentTenant.name} • Menu & Catalog`;
        case "promotions":
          return `${currentTenant.name} • Promotions`;
        case "onboarding":
          return `${currentTenant.name} • Store Profile`;
        case "setup_wizard":
          return "Launch New Restaurant";
        default:
          return "Vendor Kitchen Console";
      }
    }
    if (persona === "rider") {
      switch (riderTab) {
        case "radar":
          return "Courier Dispatch • Radar";
        case "active_job":
          return "Active Delivery Trip";
        case "earnings":
          return "Earnings & Instant Cashout";
        case "rewards":
          return "Umunthuhub Pro Tier & Badges";
        case "profile":
          return "Vehicle & Profile";
        default:
          return "Rider Logistics";
      }
    }
    if (persona === "corporate") {
      switch (corporateTab) {
        case "home":
          return "Corporate Food Program";
        case "catalog":
          return "Corporate Dining • Executive Menus";
        case "team_orders":
          return "Group Orders & Allocations";
        case "subscriptions":
          return "Daily Office Stipends";
        case "invoices":
          return "Corporate Invoicing";
        default:
          return "Corporate Meal Accounts";
      }
    }
  ...
  return (
    <header
      className={`w-full backdrop-blur-2xl border shadow-2xl rounded-2xl transition-all overflow-hidden ${
        themeMode === "dark"
    ...
    >
      ...
      <div className="px-3 sm:px-5 lg:px-6 py-2.5">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="hidden md:flex items-center gap-1.5 shrink-0">
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[10px] font-semibold transition-all duration-300 cursor-pointer transform hover:scale-105 active:scale-95 ${
                persona === "admin"
                  ? themeMode === "dark"
                    ? "bg-[#594139]/80 text-white shadow-md font-bold"
                    : "bg-[#594139] text-white shadow-md font-bold"
                  : themeMode === "dark"
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[10px] font-semibold transition-all duration-300 cursor-pointer transform hover:scale-105 active:scale-95 ${
                persona === "vendor"
                  ? themeMode === "dark"
                    ? "bg-[#24619d]/80 text-white shadow-md font-bold"
                    : "bg-[#24619d] text-white shadow-md font-bold"
                  : themeMode === "dark"
                    ? "text-[#c4c4c4] hover:bg-[#383a39] hover:text-[#f5f5f5]"
                    : "text-[#594139] hover:bg-[#f3f3f3] hover:text-[#1a1c1c]"
              }`}
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[10px] font-semibold transition-all duration-300 cursor-pointer transform hover:scale-105 active:scale-95 ${
                persona === "rider"
                  ? themeMode === "dark"
                    ? "bg-[#006c4f]/80 text-white shadow-md font-bold"
                    : "bg-[#006c4f] text-white shadow-md font-bold"
                  : themeMode === "dark"
                    ? "text-[#c4c4c4] hover:bg-[#383a39] hover:text-[#f5f5f5]"
                    : "text-[#594139] hover:bg-[#f3f3f3] hover:text-[#1a1c1c]"
              }`}
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[10px] font-semibold transition-all duration-300 cursor-pointer transform hover:scale-105 active:scale-95 ${
                persona === "corporate"
                  ? themeMode === "dark"
                    ? "bg-[#8d7168]/80 text-white shadow-md font-bold"
                    : "bg-[#8d7168] text-white shadow-md font-bold"
                  : themeMode === "dark"
                    ? "text-[#c4c4c4] hover:bg-[#383a39] hover:text-[#f5f5f5]"
                    : "text-[#594139] hover:bg-[#f3f3f3] hover:text-[#1a1c1c]"
              }`}
          ...
          <div
            className={`hidden md:flex items-center gap-2 px-2.5 py-1.5 rounded-xl border shrink-0 ${
              themeMode === "dark"
          ...
          >
            ...
            <div className="min-w-0 hidden lg:block">
              <p
                className={`font-heading font-bold text-[10px] truncate max-w-20 ${
